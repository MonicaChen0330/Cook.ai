...
from backend.app.callbacks.database_callback import DatabaseCallbackHandler
...
def get_llm_with_db_callback(job_id: int) -> ChatOpenAI:
    """Initializes the ChatOpenAI model with the database callback handler."""
    model_name = os.getenv("GENERATOR_MODEL", "gpt-4o-mini")
    db_callback = DatabaseCallbackHandler(job_id=job_id)
    # Note: To see verbose output from LangChain, you can add `verbose=True`
    return ChatOpenAI(model=model_name, callbacks=[db_callback])

def call_openai_api(llm: ChatOpenAI, prompt: str, images: List[str] = None) -> str:
    """Calls the LLM with a multimodal payload."""
...
def retrieve_chunks_node(state: ExamGenerationState) -> ExamGenerationState:
    ...
    task_id = db_logger.create_task(state['job_id'], "retrieve_chunks", "Retrieve relevant document chunks using RAG.")
...
def plan_generation_tasks_node(state: ExamGenerationState) -> ExamGenerationState:
    ...
    task_id = db_logger.create_task(state['job_id'], "plan_generation_tasks", "Analyze user query to create a generation plan.")
    start_time = time.perf_counter()
    try:
        prompt = f"Analyze the user's query to create a step-by-step generation plan.\n\n**User Query:** \"{state['query']}\"\n\nYou must respond by calling the `Plan` tool."
        llm = get_llm_with_db_callback(state['job_id'])
        ...
        duration_ms = int((time.perf_counter() - start_time) * 1000)
        db_logger.update_task(task_id, 'completed', str(state["generation_plan"]), duration_ms=duration_ms)
    except Exception as e:
...
def _generic_generate_question(state: ExamGenerationState, prompt_template: str, task_type_name: str) -> ExamGenerationState:
    ...
    task_id = db_logger.create_task(state['job_id'], f"generate_{task_type_name}", f"Generate {task_type_name} questions.")
    start_time = time.perf_counter()
    try:
        current_task = state.get("current_task", {})
        ...
        llm = get_llm_with_db_callback(state['job_id'])
        generated_part = call_openai_api(llm, final_prompt, image_data_urls)
        state["final_generated_content"].append(generated_part)
        
        duration_ms = int((time.perf_counter() - start_time) * 1000)
        db_logger.update_task(task_id, 'completed', generated_part, duration_ms=duration_ms)
    except Exception as e:
